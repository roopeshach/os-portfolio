...
const bsYearCache = new Map<number, Promise<CalendarFetchResult>>();
const adYearCache = new Map<number, Promise<CalendarFetchResult>>();
  ...
  try {
    ...
    for (let monthIndex = 0; monthIndex < 12; monthIndex += 1) {
      ...
      entries.forEach((item, index) => {
        const day = parseBsDateValue(item.np ?? '');
...
const fetchBsEventsFromSecondaryProvider = async (year: number): Promise<CalendarFetchResult> => {
  try {
    const firstMonthResponse = await fetch(`https://the-value-crew.github.io/nepali-calendar-api/data/${year}/1.json`);
    if (firstMonthResponse.status === 404) {
      return { events: [], source: 'unsupported' };
    }
    if (!firstMonthResponse.ok) {
      return { events: [], source: 'error' };
    }

    const remainingMonthRequests = Array.from({ length: 11 }, (_, index) => {
      const month = index + 2;
      return fetch(`https://the-value-crew.github.io/nepali-calendar-api/data/${year}/${month}.json`);
    });

    const remainingResponses = await Promise.all(remainingMonthRequests);
    if (remainingResponses.some((response) => !response.ok)) {
      return { events: [], source: 'error' };
    }

    const payloads = await Promise.all([
      firstMonthResponse.json() as Promise<BsSecondaryMonthResponse>,
      ...remainingResponses.map((response) => response.json() as Promise<BsSecondaryMonthResponse>),
    ]);
    const events: CalendarEvent[] = [];
...
export const fetchBsCalendarEventsForYear = async (year: number): Promise<CalendarFetchResult> => {
  const cached = bsYearCache.get(year);
  if (cached) {
    return cached;
  }

  const request = (async (): Promise<CalendarFetchResult> => {
  ...
  if (inPrimaryRange) {
    ...
    if (secondaryFallback.source === 'unsupported') {
      return primary;
    }
    return secondaryFallback;
  }

  const secondary = await fetchBsEventsFromSecondaryProvider(year);
  if (secondary.source === 'api') {
    return secondary;
  }
  return secondary.source === 'unsupported'
    ? { events: [], source: 'unsupported' }
    : secondary;
  })();

  bsYearCache.set(year, request);
  return request;
};
...
export const fetchAdCalendarEventsForYear = async (year: number): Promise<CalendarFetchResult> => {
  const cached = adYearCache.get(year);
  if (cached) {
    return cached;
  }
